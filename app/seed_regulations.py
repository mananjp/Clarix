...
SFDR_FIELDS = [
    ...
    },
    {
        ...
        "penalty_tier": "High",
        "enforcement_body": "National Competent Authority / ESMA",
        "cross_references": [{"framework": "CSRD", "field_code": "CSRD_GHG_SCOPE1", "relationship": "derived_metric"}],
        "guidance": {
        ...
        "cross_references": [
            {
                "framework": "SFDR",
                "field_code": "PERIODIC_SUSTAIN_INVEST_TARGET",
                "relationship": "equivalent_disclosure",
            },
            {"framework": "CSRD", "field_code": "CSRD_GHG_SCOPE1", "relationship": "complementary_indicator"},
        ],
        "guidance": {
    ...
    },
    {
        ...
        "cross_references": [
            {"framework": "SEC", "field_code": "SEC_GHG_SCOPE1", "relationship": "complementary_indicator"},
            {
                "framework": "SFDR",
                "field_code": "PERIODIC_SUSTAIN_INVEST_TARGET",
                "relationship": "complementary_indicator",
            },
        ],
        "guidance": {
...
DEMO_SEED_PASSWORD = "password123"
...
DEMO_USERS = [
    ...
    {
        "id": "user_auditor",
        "username": "auditor_dana",
        "email": "auditor@greenfield.com",
...
def seed_reference_data():
    """Seed safe, credential-free reference data.

    Runs in every environment (local, CI, staging, production):
      - default organization + demo products (no account credentials)
      - all regulation fields for SFDR/CSRD/SEC/UK SDR/ISSB

    Idempotent: only inserts rows that do not already exist.
    """
    logger.info("Verifying reference seed data...")
    db = SessionLocal()
    try:
        # 1. Seed default organization if not exists
        org_id = "default_org"
        ...
        # 2. Seed default products if not exists
        ...
        # 3. Seed ALL regulation fields (SFDR + CSRD + SEC + UK SDR + ISSB)
...
def seed_demo_users():
    """Seed dev/CI demo accounts. NEVER runs in production.

    Demo accounts use a hardcoded, publicly-known password
    (``DEMO_SEED_PASSWORD``). They are created only when
    ``ENVIRONMENT`` is explicitly set to a non-production value
    (e.g. ``development``, ``staging``, ``ci``). An unset
    ``ENVIRONMENT`` defaults to ``production`` and skips seeding.

    Use ``scripts/create_first_admin.py`` to provision a real admin
    in a production environment instead.
    """
    environment = os.getenv("ENVIRONMENT", "production").strip().lower()
    if environment == "production":
        logger.info(
            "Skipping demo user seeding: ENVIRONMENT=%r. Provision production users via scripts/create_first_admin.py.",
            os.getenv("ENVIRONMENT"),
        )
        return

    logger.info("Seeding demo users (ENVIRONMENT=%s)...", environment)
    db = SessionLocal()
    try:
        for u in DEMO_USERS:
        ...
        org_id = "default_org"
        db.query(User).filter(User.organization_id.is_(None)).update({User.organization_id: org_id})
        db.commit()

    except Exception as e:
        db.rollback()
        logger.error("Error seeding demo users: %s", e)
        raise e
    finally:
        db.close()
...
def seed_database():
    """Full seed for local dev/CI and manual runs: reference data + demo users."""
    seed_reference_data()
    seed_demo_users()
